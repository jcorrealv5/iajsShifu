...
function crearBarra() {
	crearGrafico();
	campos = listaProducto[cY].split("|");
	stock = +campos[1];
	var valorTotal = Math.floor(escalaH * stock);
	ctx.fillStyle = "aqua";
	ctx.fillRect(20, posY + 10, cX, 20);
	ctx.fillStyle = "white";
	ctx.fillText(cX.toString(), cX + 30, posY + 25);
	if (cX < valorTotal) cX++;
	else {
		cY++;
		cX = 0;
	...
	idAnima = requestAnimationFrame(crearBarra);
}
