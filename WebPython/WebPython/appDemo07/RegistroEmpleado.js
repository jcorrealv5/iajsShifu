...
window.onload = function(){
	...
	btnGrabarEmpleado.onclick = async function(){
		if(datosValidos()) grabarEmpleado();
	}
}

function datosValidos(){
	var c=0;
	var controles = document.getElementsByClassName("R");
	...
	for(var i=0;i<nControles;i++){
		...
		else controles[i].style.borderColor="";
	}
	if(!hayCara) imgCara.style="border:2px solid red"
	else imgCara.style="";
	return (c==0);
}
...
async function grabarEmpleado() {
    var imgBase64 = imgCara.src.replace("data:image/png;base64,","");
    var frm = new FormData();
	var token = document.getElementsByName("csrfmiddlewaretoken")[0].value;
    frm.append("csrfmiddlewaretoken", token);
	frm.append("Apellidos", txtApellidos.value);
	frm.append("Nombres", txtNombres.value);
	frm.append("DocIdentidad", txtDNI.value);
	frm.append("FechaNac", txtFechaNac.value);
	frm.append("Sexo", cboSexo.value);
	frm.append("Usuario", txtUsuario.value);
    frm.append("Foto", imgBase64);
    var rptaHttp = await fetch("GrabarEmpleado",
        {
            method: "POST",
            body: frm
        });
